...
export default function SettingsPage() {
  ...
  const applyThemeColor = (colorName: string) => {
    ...
    if (color) {
      ...
      const hslMatch = color.primary.match(/hsl$$(\d+\.?\d*)\s+(\d+\.?\d*)%\s+(\d+\.?\d*)%$$/)
  ...
  const handleExportData = () => {
    try {
      ...
      const exportFileDefaultName = `flashmaster-backup-${new Date().toISOString().slice(0, 10)}.json`
      ...
      try {
        ...
        Object.keys(importedData).forEach((key) => {
          if (key.startsWith("cards:")) {
            setItem(key.replace("cards:", "flashmaster:cards:"), importedData[key])
          }
        })
                    ...
                    <div key={color.value} className="text-center">
                      <button
                        ...
                        style={{
                          ...
                          borderColor: themeColor === color.value ? "hsl(var(--foreground))" : "transparent",
                        }}
                        onClick={() => handleThemeColorChange(color.value)}
                        title={color.name}
                      >
                        {themeColor === color.value && <Check className="h-5 w-5 text-white" />}
                      </button>
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-80">
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="custom-color">Select a color</Label>
                            <input
                              id="custom-color"
                              type="color"
                              ...
                              onChange={(e) => {
                                ...
                                setItem("flashmaster:custom-color", e.target.value)
                              }}
                            />
                          </div>
                          <Button
                            ...
                            onClick={() => {
                              toast({
                                ...
                                description: "Your custom color has been saved.",
                              })
                            }}
                          >
                            Apply Custom Color
        ...
        <TabsContent value="study">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <Label htmlFor="show-image-first">Show images before text</Label>
                <Switch
                  id="show-image-first"
                  checked={settings.showImageFirst}
                  onCheckedChange={(checked) => handleSettingChange("showImageFirst", checked)}
                />
              </div>
              <div className="flex items-center justify-between">
                <Label htmlFor="enable-spaced-repetition">Enable spaced repetition</Label>
                <Switch
                  id="enable-spaced-repetition"
                  checked={settings.enableSpacedRepetition}
                  onCheckedChange={(checked) => handleSettingChange("enableSpacedRepetition", checked)}
                />
              </div>
              <div className="space-y-2">
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) => handleSettingChange("cardsPerSession", Number.parseInt(value))}
        ...
        <TabsContent value="data">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Export and import your flashcard data</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex flex-col gap-2">
                <Button onClick={handleExportData}>Export All Data</Button>
                <p className="text-sm text-muted-foreground">
                  Download all your decks, cards, and study history as a JSON file
                </p>
              </div>
              <div className="flex flex-col gap-2">
                ...
                />
                <p className="text-sm text-muted-foreground">Import previously exported flashcard data</p>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
